...
+ Нормальное: $f(x) = frac(1, sqrt(2pi))e^(frac(-x^2,2))$
+ Коши: $f(x) = frac(1, pi(1+x^2))$
+ Лапласа: $f(x) = frac(1, sqrt(2))e^(-sqrt(2)abs(x))$
+ Пуассона: $P(x = k) = frac(lambda^k e^(-lambda), k!)$
+ Равномерное: $f(x) = cases(delim: "{", frac(1, 2sqrt(3)) & x in [-sqrt(3), sqrt(3)], 0 & x not in [-sqrt(3), sqrt(3)])$
...
1. Как вычислялось количество бинов в matplotlib? - раньше был `bins = auto` но из-за недетерминированности я поменял на `bins = sqrt`и теперь количество бинов $k = sqrt(n)$, где $n$ - количество значений в выборке.
...
- Полусумма экстремальных элементов (midrange) эффективна для распределений с ограниченным носителем (расномерное) и крайне неэффективна для распределений с тяжелыми хвостами (Коши, Пуассона)
- С увеличением объёмы выборки все состоятельные оценки сходятся к теоретическим значениям
- Медиана, midhinge и trimmed mean устойчивы: их дисперсия закономерно убывает
- В распределении Пуассона полусумма экстремальных элементов (midrange) имеет систематическое смещение вправо из-за асимметрии распределения

=== Вопросы преподавателя
- Чему равно среднее и дисперсия у распределения Коши? Их нет, т.к. хвосты слишком тяжелые и вообще интеграл не сходится
- Какие выводы можно сделать из картинок? Картинки были не говорящие и не имели надобности, потому я их убрал

== Выводы
+ Среднее арифметическое не является робастной оценкой. Для определения матожидания лучше использовать усеченное среднее или медиану
